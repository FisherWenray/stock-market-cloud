...
- Name: t1_feature_coverage.spec.ts >> Tier 1 - Feature Coverage >> T1.10: Verify correct symbols are displayed for HK
- Location: tests\e2e\specs\t1_feature_coverage.spec.ts:152:3
...
Locator: locator('[data-testid="stock-tile-0700.HK"]')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="stock-tile-0700.HK"]')
...
- banner:
  ...
  - text: "Live Mode Success. Last Updated: 11:27:33 PM"
  - button "Refresh"
- text: Market
- button "US Market"
- button "HK Market"
...
- main:
  - img: Energy Tencent +1.85% Alibaba -0.90% Finance PetroChina +1.65% Nongfu -1.15% Industrials HSBC +0.50%
```
...
> 155 |     await expect(pageObj.getStockTile('0700.HK')).toBeVisible();
  ...
  250 |     await pageObj.search('AAPL');
  251 |     const state = await pageObj.getHighlightedState('AAPL');
  252 |     expect(state).toBe('true');
  253 |     const otherState = await pageObj.getHighlightedState('MSFT');
  254 |     expect(otherState).toBe('false');
  255 |   });
```